...
# ==============================================================================
# RETICULATE ENTRY POINT
# ==============================================================================

def run_pipeline(protein_data_path, sample_labels_path):
    """
    Wrapper for R (via reticulate) to call the AI-BATS pipeline.
    Parameters
    ----------
    protein_data_path : str
        Path to the protein data CSV (from Shiny/R).
    sample_labels_path : str
        Path to the metadata/sample labels CSV (from Shiny/R).
    Returns
    -------
    dict
        Dictionary with key outputs to be returned to R:
        - processed_matrix: dict version of main output DataFrame
        - output_images: list of file paths for the generated plots
    """
    output_dir = "./data/python_output"
    ...
    # Load or prepare outputs to send back to R
    processed_matrix = pd.read_csv(f"{output_dir}/all_sample_scores.csv")  # Example output
    import glob
    image_files = glob.glob(f"{output_dir}/*.png")

    return {
        "processed_matrix": processed_matrix.to_dict(),
        "output_images": image_files
    }
